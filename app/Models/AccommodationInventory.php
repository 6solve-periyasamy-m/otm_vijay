...
class AccommodationInventory extends Model
{
    ...
    public function getAccommodationForTourAttribute()
    {
        ...
        return "{$this->accommodation->title} - {$this->accommodation->region->name}｜Check in: {$check_in} - Check out: {$check_out}｜Room Type: {$this->roomType->name} - Board Type: {$this->boardType->name}";
    }
