        ...
        return [
            ...
            'tour_name' =>
                new ColumnDefinition(
                    "$base.name",
                    Column::name('tours.name')->filterable(Tour::pluck('name'))
                ),
            'tour_description' =>
                new ColumnDefinition(
                    "$base.description",
                    Column::name('tours.description')
                ),
            'tour_base_price' =>
                new ColumnDefinition(
                    "$base.base_price",
            ...
            'tour_used_stock' =>
                new ColumnDefinition(
                    "$base.used_stock",
                    NumberColumn::raw('(SELECT COUNT(*) FROM orders WHERE tour_id = tours.id AND cancelled = FALSE;)')
                ),
            'tour_category' =>
                new ColumnDefinition(
                    "$base.category",
                    Column::name('tour_categories.category')
            ...
            'tour_from' =>
                new ColumnDefinition(
                    "$base.from",
                    DateColumn::name('tours.date_from')->filterable()
                ),
            'tour_to' =>
                new ColumnDefinition(
                    "$base.to",
                    DateColumn::name('tours.date_to')->filterable()
                ),
            'tour_final_payment' =>
                new ColumnDefinition(
                    "$base.final_payment",
                    DateColumn::name('tours.final_payment')->filterable()
                ),
        ];
    }
}
