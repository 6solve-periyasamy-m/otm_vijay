...
class AccommodationComponentRepository implements AccommodationComponentRepositoryInterface
{
    ...
    public static function getAvailableBetweenDates(Tour $tour, Carbon $dateFrom = null, Carbon $dateTo = null)
    {
        ...
        $query->select(
            ...
            DB::raw('CASE WHEN `accommodation_inventories`.`strict_check_in` = 1 THEN \'Yes\' ELSE \'No\' END  AS strict_check_in'),
            DB::raw('CASE WHEN `accommodation_inventories`.`strict_check_out` = 1 THEN \'Yes\' ELSE \'No\' END  AS check_out_confirmed'),
