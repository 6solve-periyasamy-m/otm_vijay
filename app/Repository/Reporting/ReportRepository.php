...
class ReportRepository
{
    ...
    public static function getFlightManifestReport(): array
    {
        ...
        foreach (OrderFlight::all() as $orderFlight) {
            ...
            $row->airline = $orderFlight?->tourComponent?->inventory?->component?->airline?->name;
            $row->reference = $orderFlight?->orderCustomer?->order?->booking_reference;
            $row->tour = $orderFlight?->orderCustomer?->order?->tour?->name;
            $row->class = $orderFlight?->tourComponent?->inventory->travelClass?->name;
            $row->event = $orderFlight?->orderCustomer?->order?->tour?->event?->name;
            $row->traveller_name = $orderFlight?->orderCustomer?->customer?->full_name;
            $row->traveller_phone = $orderFlight?->orderCustomer?->customer?->mobile_number;
            $row->traveller_email = $orderFlight?->orderCustomer?->customer?->email_address;
            ...
            $row->inventory_internal_notes = $orderFlight?->tourComponent?->inventory?->internal_notes;
    ...
    public static function getAbandonedBookingsReport(int|null $limit = null, bool $hideUnknown = false): array
    {
        ...
        foreach ($bookings->get() as $booking) {
            ...
            $row->created_ip = $booking->created_ip;
