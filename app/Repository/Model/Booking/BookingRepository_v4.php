...
class BookingRepository extends ModelRepository implements GeneratesFellohData
{
    ...
    {        
        ...
        /*if ($selectedTravellerCount > $bookingTravellerCount) {
            throw new IncorrectQuantityException('Selected traveller count exceeds booking travellers');
        }*/
        $components = [];
        foreach ($information as $data) {
            $tData = explode('-', $data['component']);
        ...
        // $tourComponents = $this->booking->tour
        // ->activityInventoryTours()
        // ->where('tour_component_type', 'Included')
        // ->with([
        //     'inventory.component',
        //     'inventory.ticketType',
        //     'inventory.component.seating',
        //     'inventory.component.seatingMap',
        //     'inventory.component.session',
        // ])
        // ->get();
        
        // $tourComponentIds = $this->booking->tour
        // ->activityInventoryTours()
        // ->where('tour_component_type', 'Included')
        // ->whereHas('inventory.component', function ($q) {
        //     $q->where('activity_category', ActivityCategory::MAIN);
        // })
        // ->pluck('id')
        // ->toArray();
        // dd($tourComponents, $tourComponentIds);

        // foreach ($tourComponents as $tourComponent) {
        //     if (in_array($tourComponent->tour_component_type, ['Add-on', 'Upgrade'], true)) {
        //         continue;
        //     }
        ...
        //     $id = $tourComponent->id;
        //     $tourComponentData[] = $id;
        // }

        foreach ($this->booking->travellers as $traveller) {            
            foreach ($traveller->activities as $component) {
                dd($traveller->activities()->where('activity_inventory_tour_id', '=', $component->activity_inventory_tour_id)->get());
                //dd($component);
                echo $component->activity_inventory_tour_id."<br>";
            }
        }
        dd($this->booking->travellers);
        foreach ($components as $data) {
            $quantity = $data['quantity'];
            /** @var InventoryTourRepository $component */
            $component = $data['component'];            
            ...
            foreach ($this->booking->travellers as $traveller) {
                if ($quantity <= 0) { break; }
                //dd($traveller->role, BookingTravellerRole::NOT_TRAVELLING);
                if ($traveller->role !== BookingTravellerRole::NOT_TRAVELLING) {
                    $component->grantToBookingTraveller($traveller);
                    $quantity--;
                }
