...
class TransportTransforms implements TransportTransformsInterface
{
    ...
    public static function getSelectedInventory($id)
    {
        ...
        $data['text'] = $inventory->transport->name . ' - ' . $inventory->transport->transportType->name . ' - ' . $inventory->travelClass->name . ' - ' . $inventory->transport->departureAddress->name . ' to ' . $inventory->transport->arrivalAddress->name;
        return $data;
    }
