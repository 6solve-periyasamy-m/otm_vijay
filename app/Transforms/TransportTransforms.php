...
class TransportTransforms implements TransportTransformsInterface
{
    ...
    public static function getSelectInventory($filter)
    {
        ...
        foreach (TransportInventory::all() as $inventory) {
            ...
            $subData['text'] = $inventory->transport->name . ' - ' . $inventory->transport->transportType->name . ' - ' . $inventory->travelClass->name . ' - ' . $inventory->departureAddress->name . ' to ' . $inventory->arrivalAddress->name;
            if (str_contains(strtolower($subData['text']), strtolower($filter))) $data['results'][] = $subData;
        }
        return $data;
    }
