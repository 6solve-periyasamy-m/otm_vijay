@php
    $fieldList = App\Repository\Reporting\ReportFieldRepository::getFieldsFromParent('customer');
    $customerFieldList = current(array_filter($fieldList, fn($item) => $item['type'] === 'customer'));
    $selectedFields = explode(',', setting('system.customer.fields'));
    $selectedOrderCustomerFields = explode(',', setting('system.order.customer.fields'));
@endphp
...
<x-admin.section.accordion>
    <x-slot:title>System Configuration</x-slot:title>
    <x-admin.section.card>
        <div class="d-flex justify-content-between">
            <div>
                <a href="{{ route('settings.template') }}" class="btn btn-info">{{ Icon::template() }} Editor
                    Templates</a>
            </div>
            <div>
