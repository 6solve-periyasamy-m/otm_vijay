...
<div class="form-group col-12 {{ isset($width) ? 'col-xl-' . $width : '' }} {{ $divClasses ?? "" }}">
    ...
    <div class="d-flex flex-wrap justify-content-center flex-column upgrade-wrapper" style="gap: 5px">
        ...
        <span class="hide-on-mobile">
            ...
            <a href="{{ $createRoute }}" target="{{ $target ?? '_blank' }}" class="btn btn-success" onclick="{{$onclick ?? ''}}">+</a>
            @endif
            <a href="{{ $purchaseRoute }}" target="{{ $targetPurchase ?? '_blank' }}" class="btn btn-primary" onclick="{{$onclickPurchase ?? ''}}">$</a>
        </span>
