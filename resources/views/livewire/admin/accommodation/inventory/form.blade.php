<x-admin.section.card>
    <div class="row">
        ...
        <x-livewire.input type="datetime-local" wire:model="inventory.check_in" wire:change="updatedInventoryCheckIn" width="4" label="Check In" id="check_in" min="{{ now()->format('Y-m-d\TH:i') }}"/>
        <x-livewire.input.checkbox wire:model="inventory.check_in_time_confirmed" width="2" label="Confirmed?" />
        <x-livewire.input type="datetime-local" wire:model="inventory.check_out" width="4" label="Check Out" id="check_out" :min="$minEndDate"/>
