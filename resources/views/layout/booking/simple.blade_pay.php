...
<head>
    ...
    <script type="text/javascript">
        let elements;
        let paymentElement;
        let currentStripeIntent = null;

        function isMobile() {
            return window.innerWidth <= 767;
        }

        function movePaymentRoot() {
            const root = document.getElementById('payment-root');
            const target = isMobile()
                ? document.getElementById('payment-mobile-slot')
                : document.getElementById('payment-desktop-slot');

            if (root && target && !target.contains(root)) {
                target.appendChild(root);
                root.style.display = 'block';
            }
        }
        window.addEventListener('resize', movePaymentRoot);
        ...
        window.addEventListener('popupStripeCheckout', async (event) => {
            ...
            if (event.detail.checkout !== null) {
                ...
                let keys = await fn();
                currentStripeIntent = keys.intent;
                ...
                document.getElementById('payment-root').style.display = 'block';

                document.getElementById('pay-button').addEventListener('click', async () => {
                    const errors = document.getElementById('confirm-errors');
                    errors.textContent = '';
                    await elements.submit();
                    const { error: pmErr, paymentMethod } = await stripe.createPaymentMethod({ elements, params: {} });

                    if (pmErr) {
                        console.log(pmErr.message);
                        //errors.textContent = pmErr.message;
                        return;
                    }
...
<body>
        ...
        <section class="top-block">
            <div class="row">
                <div class="top-nav-sec">
                    ...
                    @endif
                    <div class="head">
                        <h1 class="head-title">@if(!empty($return ?? null))
                                Checkout
                            @else
                                Secure Booking
                            @endif
    ...
    <div id="payment-root" wire:ignore style="display:none">
        {{-- Stripe --}}
        <div id="stripe-root">
            <div id="stripe-container"></div>
            <div id="surcharge-warning" hidden>
                A card surcharge of <span id="surcharge-percent"></span>% (<span id="surcharge-amount"></span>) will be added to card transactions
            </div>
            <button type="button" id="pay-button" class="next-button">PAY</button>
