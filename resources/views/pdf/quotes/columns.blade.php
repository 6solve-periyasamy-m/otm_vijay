        ...
        <div class="header">
            ...
            <div class="flex-container">
                <div class="flex-items">
                    <span class="metadata">Website:</span> <a class="site-info-padding"
                                                              href="{{ $brand->url ?? setting('company.url', URL::to('/')) }}">{{ $brand->url ?? setting('company.url', URL::to('/')) }}</a>
                    <br/><span class="metadata">Email:</span> <a class="site-info-padding"
                                                                 href="mailto:{{ $brand->email ?? setting('company.contact.email', 'Email not set') }}">{{ $brand->email ??  setting('company.contact.email', 'Email not set') }}</a>
                    <br/><span class="metadata">Telephone:</span> <a class="site-info-padding"
                                                                     href="tel:{{ $brand->phone ?? setting('company.contact.phone', 'Phone number not set') }}">{{ $brand->phone ?? setting('company.contact.phone', 'Phone number not set') }}</a>
                </div>
                <div class="flex-items metadata-wrapper">
                    @if($paying > 0)
                        <div class="metadata divider">Paying Travellers<br/><span
                                    class="metadata-text">{{ $paying }}</span></div>
                    @endif
                    @if($travelling > 0)
                        <div class="metadata divider">Non-Paying Travellers<br/><span
                                    class="metadata-text">{{ $travelling }}</span></div>
                    @endif
                    <div class="metadata divider">Quote Ref.<br/><span class="metadata-text">{{ $quote->ref }}</span>
                    </div>
                </div>
            </div>
        </div>
            ...
            <table class="order-table center">
                ...
                @if(sizeof($quote->accommodation))
                    ...
                    @foreach($quote->repository->getAccommodationForInvoice() as $component)
                        <tr>
                            <td class="date-double">
                                <div class="order-table-description">{{ f_datetime($component->getInventory()->getStartTime()) }}
                                    to {{ f_datetime($component->getInventory()->getEndTime()) }}</div>
                            </td>
                            <td class="short-description">
                                <div class="order-table-description">{{ $component->getShortDescription() }}
                ...
                @endif
                <tr>
                    <td colspan="3" class="metadata right-text">
                        Total: {{ f_currency($quote->repository->getTotalCost($paying)) }}</td>
                </tr>
                </tbody>
            </table>
        </div>
