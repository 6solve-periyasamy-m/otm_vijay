...
<div class="row">
    <div class="col-md-2">
        <x-admin.section.card>
            @if($editable)
            <form method="post" action="{{ route('users.avatar', ['user' => $user,]) }}" enctype="multipart/form-data">
            @endif
                <div class="mt-4 text-center">
                    <div class="overlay-container">
                        <label for="avatar" class="my-auto">
                            <img src="{{ $user->avatar_url }}" class="rounded-circle {{ $editable ? 'hover-upload' : '' }} image med-small" alt="{{ $user->name }}"/>
                            @if($editable)
    ...
    <div class="col-md-10">
        <x-admin.section.card>
                <form method="post" action="{{ route('users.update', ['user' => $user,]) }}" class="row">
                    ...
                    <x-livewire.input disabled="{{!$editable}}" name="name" value="{{ $user->name }}" label="Name" width="6" />
                    <x-livewire.input disabled label="Roles" value="{{ $user->roles->implode('title', ', ') }}" width="6" />
                    <x-livewire.input disabled="{{!$editable}}" name="telephone" value="" label="Contact Number" width="{{ $editable ? 5 : 6 }}" />
                    <x-livewire.input disabled="{{!$editable}}" name="email" value="{{ $user->email }}" label="Email" width="{{ $editable ? 5 : 6 }}" />
                    @if($editable)
...
<x-admin.section.card>
    ...
    <form action="{{ route('users.password', ['user' => $user,]) }}" method="post" class="row">
        ...
        <x-livewire.input type="password" name="new_password_confirmation" label="Confirm New Password" width="3" />
        <div class="col-xl-2 flex justify-center my-auto">
