            ...
            <x-slot:header>
                <div class="d-flex justify-content-between align-items-center">
                    <h4 class="fw-bold mb-0">
                        {{ __('quotes.view.cards.installments.header') }}
                    </h4>
            ...
            <table class="datatable table table-striped" id="schedule-table">
                ...
                <tbody>
                    <tr>
                        ...
                        <td>{{ fr_currency($quote->getDepositAmount(), $quote->currency) }}
                            ({{$quote->getDepositPercentage()}}%)</td>
                        <td class="installment-cell" base="{{ $quote->getDepositAmount() }}">{{
                            $quote->getDepositAmount() }}</td>
                        <td>
                    ...
                    <tr>
                        ...
                        <form id="installment-{{ $installment->id }}-delete" class="d-none" method="post"
                            action="{{ route('quotes.installments.delete', ['quote' => $quote, 'installment' => $installment,]) }}">
                            @csrf</form>
                    </tr>
                    @endforeach
                    <tr>
                        <td>{{ __('quotes.view.cards.installments.types.remaining') }}</td>
                        <td data-order="{{$quote->final_payment->format('Y-m-d')}}">{{ f_date($quote->final_payment) }}
                        </td>
                        <td>{{ fr_currency($quote->remaining, $quote->currency) }} ({{ $quote->getRemainingPercentage()
                            }}%)</td>
                        <td class="installment-cell" base="{{ $quote->remaining }}">{{ fr_currency($quote->remaining,
                            $quote->currency) }}</td>
                        <td>
                            <a href="{{ route('quotes.edit', ['quote' => $quote,]) }}"
                                class="btn btn-outline-success btn-sm mb-1" title="Edit">
    ...
    <div class="col-xl-12">
        <x-admin.section.card>
            <div class="d-flex justify-content-between align-items-center pb-3">
                <div class="fw-bold">
                    <h4 class="fw-bold">{{ __('quotes.view.cards.sections.header') }}</h4>
                ...
                <div class="d-flex gap-2">
                    ...
                    <a href="{{ route('quotes.section.hide', ['quote' => $quote]) }}" class="btn btn-info text-white">
                        {{ Icon::hide() }} Hide All
                    </a>

                    <a href="{{ route('quotes.section.create', ['quote' => $quote]) }}"
                        class="btn btn-success text-white">
                        {{ Icon::create() }} New Section
                    </a>
                </div>
            </div>
            ...
            <table class="table table-striped sent-quotes" id="sent-quotes-table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ $sent->subject }}</td>
                        <td class="actions-3">
                            <a href="javascript:void(0);" class="btn btn-outline-success btn-sm mb-1"
                                title="View sent version details"
                                onclick="Livewire.emit('showSentQuoteInfo', {{ $sent->id }})">{{ Icon::list() }}</a>
