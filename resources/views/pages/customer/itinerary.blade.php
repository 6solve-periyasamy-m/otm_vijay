...
@push('footer-stack')
    <script>
        const route = "{{ route('customer.itinerary', ['reference' => 'reference']) }}"
        function onOrderChange(selector) {
            window.location = route.replaceAll('reference', $(selector).val());
        }
    </script>
@endpush
@section('content')
        ...
        <div class="form-group d-flex align-items-center">
            ...
            <select class="form-select order-select" onchange="onOrderChange(this);" id="booking_reference">
                @foreach($orders as $selector)
                    <option value='{{ $selector->booking_reference }}'>{{ $selector->tour->name }} ({{ $selector->booking_reference }})</option>
                @endforeach
            </select>
        </div>
    </form>
