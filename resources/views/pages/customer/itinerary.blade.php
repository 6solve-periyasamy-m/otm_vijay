                    ...
                    <select class="form-select order-select" onchange="onOrderChange(this);" id="booking_reference">
                        @foreach($orders as $selector)
                            <option value='{{ $selector->booking_reference }}' @if($selector->id == $order->id) selected
                                    @endif @if($selector->cancelled) disabled @endif>{{ $selector->booking_reference }} @if($selector->cancelled)
                                    (Cancelled)
                                @endif - {{ $selector->tour->name }}</option>
                        @endforeach
                    </select>
                    <a href="{{ route('customer.invoice', ['reference' => $order->booking_reference]) }}"
                       target="_blank" class="m-l-20 invoice btn btn-primary">Invoice</a>
                    @if ($order->has_atol)
                        <a href="{{ route('customer.atol', ['reference' => $order->booking_reference]) }}"
                           target="_blank" class="m-l-5 invoice btn btn-secondary">ATOL Certificate</a>
                    @endif
                </div>
            </form>
        </div>
        ...
        <div class="col-10 container-fluid">
            <div class="card">
                <div class="card-body">
                    ...
                    <hr class="splitter">
                    <form action="{{ route('customer.notes.update', ['reference' => $order->booking_reference, 'orderCustomer' => $orderCustomer,]) }}"
                          method="post">
                        @csrf
                        @if($order->repository->isLeadBooker(\App\Repository\Authentication\CustomerAuthenticationRepository::getCustomer()))
